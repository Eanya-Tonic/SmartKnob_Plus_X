...
void handle_ota(String releaseUrl)
{
    ...
    String url = _fetch_url_via_redirect ? get_updated_firmware_url_via_redirect(releaseUrl, &client) : get_updated_firmware_url_via_api(releaseUrl, &client);
...
void update_progress(int currentyReceiced, int totalBytes)
{
    ...
    str = "Progress: " + String(100.0 * currentyReceiced / totalBytes) + "%";
}
