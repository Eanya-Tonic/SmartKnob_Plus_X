 ...
 */
void Page::wifiDialView::SetPlaygroundMode(int16_t mode)
{
...
void Page::wifiDialView::UpdateView(PlaygroundInfo *info)
{
	...
	if (millis() - last_publish > 5)
	{
		...
		if (info->is_wifi_connected)
		{
			if (!HAL::return_UDP())
			{
				String str = "IP: " + HAL::return_IP();
				lv_label_set_text(m_ui.label, str.c_str());
			}
			else
			{
				...
				{
					updateTime = millis();
					tmpSecond = HAL::updateTime();
					updateTime -= tmpSecond * 1000; // 解决时间偏移的问题
				}
				String str;
				int hour = HAL::return_hour();
				int minute = HAL::return_minute();
...
void wifiDialView::Create(lv_obj_t *root)
{
	...
	lv_label_set_text(m_ui.label, "");
	lv_obj_align(m_ui.label, LV_ALIGN_BOTTOM_MID, 0, -55);
}

void wifiDialView::Delete()
{
