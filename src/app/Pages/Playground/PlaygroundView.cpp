...
void PlaygroundView::DefaultView(void)
{
	...
	if (HAL::LightMode())
	{
		lv_meter_set_scale_ticks(ui.meter, ui.scale_pot, 73, 2, 0, lv_color_white());
		lv_meter_set_scale_major_ticks(ui.meter, ui.scale_pot, 0, 0, 20, lv_color_white(), 10);
	}
	else
	{
...
void PlaygroundView::OnOffView(void)
{
	...
	if (HAL::LightMode())
	{
		lv_meter_set_scale_ticks(ui.meter, ui.scale_pot, 3, 2, 0, lv_color_black());
		lv_meter_set_scale_major_ticks(ui.meter, ui.scale_pot, 2, 4, 20, lv_color_black(), 10);
	}
	else
	{
		lv_meter_set_scale_ticks(ui.meter, ui.scale_pot, 3, 2, 0, lv_color_white());
		lv_meter_set_scale_major_ticks(ui.meter, ui.scale_pot, 2, 4, 20, lv_color_white(), 10);
	}
...
void PlaygroundView::BoundZeroView(void)
{
	...
	if (HAL::LightMode)
	{
		lv_meter_set_scale_ticks(ui.meter, ui.scale_pot, 13, 2, 0, lv_color_black());
		lv_meter_set_scale_major_ticks(ui.meter, ui.scale_pot, 12, 4, 20, lv_color_black(), 10);
	}
	else
	{
...
void PlaygroundView::Create(lv_obj_t *root)
{
	...
	if (HAL::LightMode())
		lv_style_set_bg_color(&style.meter, lv_color_white());
	else
